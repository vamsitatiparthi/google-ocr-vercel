...
function buildUniversalStructured(text = '', meta = {}) {
  ...
  const tables_summary = (detectedTables || []).map((t, ti) => {
    ...
    const data_rows = rowsArr.length;
    // total_rows: include header row if headers are present
    const total_rows = (headersArr && headersArr.length ? 1 : 0) + data_rows;
    const colCount = t.colCount != null ? t.colCount : (headersArr.length || (rowsArr[0] ? Object.keys(rowsArr[0]).length : 0));
    // raw_data: convert each row object into array ordered by headersArr; if headers missing, use Object.values
    const raw_data = rowsArr.slice(0, 2000).map(r => {
      if (!r) return [];
      if (Array.isArray(r)) return r.slice(0, colCount);
      // r is object: map headers order
      if (headersArr && headersArr.length) return headersArr.map(h => (r[h] == null ? '' : String(r[h])));
      return Object.values(r).map(v => (v == null ? '' : String(v)));
    });

    return {
      table_number: t.tableId || (ti + 1),
      page: t.page || null,
      total_rows,
      data_rows,
      columns: colCount,
      headers: headersArr,
      raw_data: raw_data.slice(0, 5) // keep small preview like before
    };
  });
  const items = [];
