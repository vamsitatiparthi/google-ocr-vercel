...
function buildUniversalStructured(text = '', meta = {}) {
  ...
  const tables_summary = (tables || []).map((t, ti) => {
    const rowsArr = t.rows || [];
    const headersArr = t.headers || (rowsArr[0] ? Object.keys(rowsArr[0]) : []);
    const rowCount = rowsArr.length;
    const colCount = headersArr.length || (rowsArr[0] ? Object.keys(rowsArr[0]).length : 0);
    const rows_preview = rowsArr.slice(0, 5);
    return {
      table_id: ti + 1,
      // page is not available from simple text extraction; keep null if unknown
      page: t.page || null,
