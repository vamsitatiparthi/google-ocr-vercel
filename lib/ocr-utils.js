...
function extractTables(text = '') {
  ...
  for (const b of blocks) {
    ...
    if (maxCols >= 2 && colCounts.filter(c => c >= 2).length >= Math.max(1, Math.floor(lines.length * 0.4))) {
      // choose header row as the first row with the maximum number of columns
      let headerIdx = colCounts.findIndex(c => c === maxCols);
      if (headerIdx < 0) headerIdx = 0;
      const headers = (splitted[headerIdx] && splitted[headerIdx].length >= 2) ? splitted[headerIdx] : Array.from({ length: maxCols }, (_, i) => `col_${i+1}`);
      // data rows come after header; skip rows before header (they're often notes like 'Place:' lines)
      const dataRows = splitted.slice(headerIdx + 1).filter(r => r.length >= 1);
